...
Every [vanilla movement tip](/guide#movement) also applies in Overkill. However there is one key aspect making the game way faster - **dodging**. But let's review the basics first:

Bunny Hopping
-------------

[Same as vanilla](/guide#bunny-hopping) - instead of just walking around you should always jump because it's much faster. Basically keep the `jump` key (by default `space bar`) pressed all the time. The are only two exceptions to this rule:
...
- accelerating - shoot the floor or wall [slightly behind you](/guide#wall-blastering) for a noticeable boost.
